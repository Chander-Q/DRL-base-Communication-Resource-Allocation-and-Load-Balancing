...
def make_eval_env(all_args):
    def get_env_fn(rank):
        def init_env():
            from envs.env_discrete import DiscreteActionEnv_Eval
            env = DiscreteActionEnv_Eval()
            env.seed(all_args.seed + rank * 1000)
            return env
        return init_env
    return DummyVecEnv([get_env_fn(i) for i in range(all_args.n_rollout_threads)])
...
def main(args):
    ...
    # test log dir
    run_dir = (
        Path(os.path.split(os.path.dirname(os.path.abspath(__file__)))[0] + "/test_results")
        / all_args.env_name
    ...
    # 创建测试环境
    eval_envs = make_eval_env(all_args) if all_args.test_mode else None
